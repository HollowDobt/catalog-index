...
class ArxivClient(AcademicDBClient):
    ...
    def _health_check(self) -> None:
        ...
        try:
            ...
            if not response:
                ...
                raise ArxivInitConnectError(
                    "ArxivClient initalize error. This often happens when base_url or end_point is incorrect."
                )
            ...
            try:
                ...
                raise ArxivInitConnectError(
                    "ArxivClient initalize error but test succeed. Please check if the end_point is correct."
                )

            except Exception as exc:
                raise ArxivConnectError(
                    f"Unable to connect to arxiv.org. You may have entered the wrong base_url. Details: {exc}"
                ) from exc
    ...
    def search_get_metadata(self, query: str, max_num: int) -> List[Dict[str, Any]]:
        ...
        if max_num <= 0:
            raise ValueError(
                "Illegal parameters passed in, 'max_num' must be greater than 0."
            )
    ...
    def single_metadata_parser(self, meta_data: Dict[str, Any]) -> str:
        ...
        if pdf_url is None:
            raise ArxivConnectError(
                "Unable to obtain the specified PDF file. Please check whether the network connection or metadata is normal."
